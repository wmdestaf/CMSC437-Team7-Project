...
print "\t\t\"weight\": $weight,\n";
print "\t},\n";
print "\t\"currently-admitted\": 1,\n";
print "\t\"medical-history\": {\n";
my $history_ct = rand(6) + 1;
for(my $i = 0; $i < $history_ct; $i++) {
	print "\t\t\"" . generateDate() . "\": \"" . $events[rand(scalar @events)] . "\",\n";
}
print "\t},\n";
my $cur_med_ct = rand(5) + 1;
print "\t\"current-medications\": [\n";
for(my $i = 0; $i < $cur_med_ct; $i++) {
	print "\t\t\"" . $drugs[rand(scalar @drugs)] . "\",\n";
}
print "\t],\n";
print "\t\"former-medications\": [\n";
my $prev_med_ct = rand(6) + 2;
for(my $i = 0; $i < $prev_med_ct; $i++) {
	print "\t\t\"" . $drugs[rand(scalar @drugs)] . "\",\n";
}
print "\t],\n";
print "\t\"conditions\": [\n";
my $cond_count = rand(3) + 1;
for(my $i = 0; $i < $cond_count; $i++) {
	print "\t\t\"" . $conditions[rand(scalar @conditions)] . "\",\n";
}
print "\t],\n";
